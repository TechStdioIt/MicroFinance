...
export default function TellerPage() {
  ...
  const handleInitiateTxn = (e: React.FormEvent) => {
    ...
    // If withdrawal, trigger our mandatory biometric authentication!
    if (txnType === 'WITHDRAWAL' && selectedAccountType === 'SAVINGS') {
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <form onSubmit={handleInitiateTxn} className="lg:col-span-7 glass-card rounded-3xl p-7 border border-slate-200 dark:border-slate-800 space-y-6">
              ...
              {selectedAccountType === 'SAVINGS' ? (
                <>
                  ...
                  >
                    ...
                    <span className="text-[9px] bg-black/20 px-1 rounded uppercase font-mono">Biometric</span>
                  </button>
                </>
              ) : selectedAccountType === 'DPS' ? (
                <div className="col-span-2 py-2.5 px-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-400 font-bold text-xs">
                  Recurring Monthly DPS Installment Deposit
                </div>
              ) : (
                <div className="col-span-2 py-2.5 px-4 rounded-xl bg-teal-500/10 border border-teal-500/30 text-teal-700 dark:text-teal-400 font-bold text-xs">
          ...
          {/* Amount input */}
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">4. Transaction Amount (BDT)</label>
            <input
              type="number"
              min={10}
              required
